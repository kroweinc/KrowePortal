...
export async function getQuoteByToken(token: string): Promise<PublicQuote | null> {
  ...
  // (builder-internal AI notes). source_prd_id is also internal so it's omitted.
  const { data } = await admin
    .from("quotes")
    .select(
      "id, project_id, title, status, content, token, token_expires_at, token_revoked_at, sent_at, signed_by_name, signed_at, signature_consent, accepted_at, rejected_at, rejection_note, created_at, updated_at, project:projects(name, owner_id, owner:profiles!owner_id(display_name))"
    )
    .eq("token", token)
    .maybeSingle();
  ...
  const { project: _p, token_expires_at: _te, token_revoked_at: _tr, ...quoteRow } = row;
