...
- **Buttons:** four variants only (Primary / Secondary / Ghost / Destructive). Max one primary action per view (2 buttons max on screen).
- **Focus — split by the verb, never a judgment call.** *Click/pick/toggle it* (button, link, `select`, checkbox, radio) → 4px orange halo + 1px `--primary`. *Type into it* (`input`, `textarea`, composers, search) → 1px `var(--border-focus)`, **no halo, never orange** — the ring reads as an error on an idle field. A bordered shell around a borderless control carries `.krowe-field-shell`: the shell draws the one treatment, the control inside draws none. Never hand-roll a `:focus-within` rule plus an inner zeroing rule, and never add an orange focus to a text input in a feature stylesheet.
